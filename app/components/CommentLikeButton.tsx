    ...
    async function fetchScore() {
      if (isUsingBackend() && answerId) {
        try {
          ...
          console.error("[CommentLikeButton] Error fetching score:", e);
        }
      }
    }
    fetchScore();
  ...
  const handleLike = async () => {
    ...
    try {
      if (isUsingBackend() && answerId) {
        ...
        console.info(`[CommentLikeButton] ${newLiked ? "評価しました" : "評価を取り消しました"} (Answer ID: ${answerId})`);
      }
    } catch (error) {
      console.error("[CommentLikeButton] エラー:", error);
      // エラー時は元の状態に戻す
      setLiked(!newLiked);
      setCount(count);
